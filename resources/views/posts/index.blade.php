        ...
        <div class="contaier-fluid">
            ...
            <div id="other_pos"> 
			  @foreach($posts as $post)
				<div href="/post/{{$post->slug}}" class="other_post col-sm-6 col-md-4 pan_pad">
					<div class="pane article_title hidden-lg hidden-md">  
              <h4><a href="/post/{{$post->slug}}">{{ str_limit($post->title, 65) }}</a></h4>
              <h6 class="text-right small">{{$post->created_at->diffForHumans()}}</h6>
          </div>
          <div class="pane article_title hidden-xs hidden-sm">  
							<h4><a href="/post/{{$post->slug}}">{{ str_limit($post->title, 52) }}</a></h4>
							<h6 class="text-right small">{{$post->created_at->diffForHumans()}}</h6>
          </div>
            ...
            <a href="/post/{{$post->slug}}">
							...
							<p class="sp  article_description" style="margin-top:0px;">{{ str_limit(strip_tags($post->body), 145) }} <a href="post/{{$post->slug}}" class="btn btn-xs btn-primary">Read More...</a> </p>    
